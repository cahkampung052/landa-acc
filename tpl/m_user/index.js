app.controller("userCtrl", function($scope, Data, $rootScope) {
    ...
    $scope.callServer = function callServer(tableState) {
        ...
        Data.get("acc/appuser/index", param).then(function(response) {
    ...
    Data.get("acc/appakses/index").then(function(data) {
    ...
    $scope.save = function(form) {
        $scope.loading = true;
        Data.post("acc/appuser/save", form).then(function(result) {
    ...
    $scope.trash = function(row) {
        if (confirm("Apa anda yakin akan MENGHAPUS item ini ?")) {
            row.is_deleted = 1;
            Data.post("acc/appuser/saveStatus", row).then(function(result) {
