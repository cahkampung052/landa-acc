app.controller('l_rekappiutangCtrl', function($scope, Data, $rootScope, $state) {
     ...
     */
    $scope.viewPiutang = function(row) {
        var data = {
            export: 0,
            print: 0,
    ...
    $scope.view = function(is_export, is_print) {
        ...
        if (is_export == 0 && is_print == 0) {
            ...
            Data.get('site/base_url').then(function(response) {
                window.open(response.data.base_url + "api/acc/l_rekap_piutang/laporan?" + $.param(param), "_blank");
            });
        }
    };
});