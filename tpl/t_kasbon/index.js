app.controller('kasbonCtrl', function ($scope, Data, $rootScope, $uibModal, Upload) {
    var tableStateRef;
    var control_link = "acc/t_kasbon";
    var master = 'Transaksi Kasbon';
    $scope.formTitle = '';
    $scope.displayed = [];
    $scope.base_url = '';
    $scope.is_edit = false;
    ...
    Data.get('acc/m_customer/getKaryawan').then(function (data) {
        $scope.listKaryawan = data.data.list;
    });
    
    Data.get('site/listcabang').then(function (response) {
        $scope.listCabang = response.data;
    });
    ...
    $scope.create = function () {
        ...
        $scope.form = {};
        if($scope.listCabang.length > 0){
            $scope.form.cabang = $scope.listCabang[0];
        }
        if($scope.listKaryawan.length > 0){
            $scope.form.karyawan = $scope.listKaryawan[0];
        }
