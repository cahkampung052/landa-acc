app.controller('pembayaranhutangCtrl', function ($scope, Data, $rootScope, $uibModal, Upload) {
    var tableStateRef;
    var control_link = "acc/t_pembayaran_hutang";
...
//        minMode: 'year'
    ...
    $scope.kalkulasi = function () {
        ...
        $scope.form.totalHutang = parseInt($scope.form.total_bayar);
    ...
    $scope.bayarIni = function (index, cek) {
        ...
        if (cek == true) {
            $scope.detHutang[index].bayar = $scope.detHutang[index].sisa;
        } else {
            $scope.detHutang[index].bayar = 0;
        }
    }

    $scope.getDetail = function (id) {
        Data.get("acc/t_pembayaran_hutang/view?id=" + id).then(function (response) {
            $scope.form.m_akun_denda_id = response.data.akun_denda;
            $scope.detHutang = response.data.detail;
            $scope.kalkulasi();
        });
    };

    $scope.removeRow = function (paramindex) {
        var comArr = eval($scope.detHutang);
        if (comArr.length > 1) {
            ...
            if (val.akun_hutang != undefined && val.bayar > 0) {
                if (index > 0) {
                    if (listJurnal[index - 1] && listJurnal[index - 1].akun.id == val.akun_hutang.id) {
                        ...
                        listJurnal[index - 1].keterangan += "</br>Pembayaran Hutang (" + val.kode + ")";
                    } else {
                        listJurnal[index] = {
                            akun: val.akun_hutang,
                            tipe: "debit",
                            debit: val.bayar,
                            kredit: 0,
    ...
    $scope.callServer = function callServer(tableState) {
        ...
        var param = {
            limit: limit,
            offset: offset,
            m_supplier_id: $scope.form.supplier != undefined ? $scope.form.supplier.id : undefined,
        };
        /** set sort and order */
