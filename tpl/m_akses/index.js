app.controller("aksesCtrl", function($scope, Data, $rootScope) {
    ...
    $scope.callServer = function callServer(tableState) {
        ...
        Data.get("acc/appakses/index", param).then(function(response) {
    ...
    $scope.trash = function(row) {
        ...
        if (confirm("Apa anda yakin akan MENGHAPUS item ini ?")) {
            data.is_deleted = 1;
            Data.post("acc/appakses/saveStatus", data).then(function(result) {
                if (result.status_code == 200) {
    ...
    $scope.restore = function(row) {
        if (confirm("Apa anda yakin akan MERESTORE item ini ?")) {
            row.is_deleted = 0;
            Data.post("acc/appakses/saveStatus", row).then(function(result) {
                $scope.cancel();
            });
        }
    };
