...
$app->get('/acc/m_format_kode/index', function ($request, $response) {
    $params = $request->getParams();
    $db = $this->db;

//    DEKLARASI AKUN PEMETAAN
//    $akunpeta = ["Pengimbang Neraca", "Laba Rugi", "tes"];
    $arr = getMasterSetting();
...
$app->post('/acc/m_format_kode/save', function ($request, $response) {

    $params = $request->getParams();
    $data = $params;
//    print_r($data);die();
    $sql = $this->db;
    $model = $sql->update("acc_m_setting", $data, ["id"=>1]);
